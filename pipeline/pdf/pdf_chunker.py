...
def _save_chunked_document(chunked: ChunkedDocument, out_dir: Path, source_json: Path) -> Path:
    ...
    if chunked.layout_analysis:
        out_dict["layout_analysis"] = chunked.layout_analysis
    if chunked.document_blocks:
        out_dict["document_blocks"] = chunked.document_blocks

    with out_path.open("w", encoding="utf-8") as fh:
        json.dump(out_dict, fh, ensure_ascii=False, indent=2)
    return out_path
...
_BANNER_PATTERNS: tuple[re.Pattern, ...] = (
    ...
    # Letterhead / header lines common in university notifications
    re.compile(r"^\s*(?:OFFICE\s+OF\s+THE|Office\s+of\s+the)\b", re.I),
    re.compile(r"^\s*Accredited\s+with\b", re.I),
    # Reference/notification number lines: "No. 318/E9/KU/2025 Date:30-06-2025"
    re.compile(r"^\s*No\.\s*[\d/A-Za-z-]+\s+Date\s*[:\-]?\s*[\d\-/]+\s*$", re.I),
    re.compile(r"^\s*KAKATIYA\s+UNIVERSITY\b", re.I),
    # Signature lines at end of documents
    re.compile(r"^\s*(?:Addl\.?|Additional|Joint|Deputy|Asst\.?)\s+(?:CONTROLLER|REGISTRAR|DIRECTOR)\b", re.I),
    re.compile(r"^\s*Copy\s+to\s*[:\-]?\s*$", re.I),
)
...
_PROTECTED_PATTERNS: tuple[re.Pattern, ...] = (
    ...
    # Strict course code: letters + optional dash + 3-5 digits, NOT followed by / and 4-digit year
    re.compile(r"\b[A-Z]{2,6}\s*-?\s*\d{3,5}[A-Z]{0,5}\b(?!/\s*\d{4})"),
...
def _is_banner_or_table_noise(text: str, dynamic_banners: frozenset[str] = frozenset()) -> bool:
    ...
    if normalized in dynamic_banners:
        return True
    # Strip pipe-delimited table rows ("[TABLE N]" expansions like "X | Y")
    if _TABLE_ROW_FRAGMENT.search(normalized):
        return True
    return False
...
# ---------------------------------------------------------------------------
# Tokens / patterns that are NEVER real headings
# ---------------------------------------------------------------------------

# Single letters (grade labels like A, B, C, S, O, P, F)
_SINGLE_LETTER = re.compile(r"^[A-Za-z]$")

# Roman numerals only (I, II, III, IV, V, VI, VII, VIII, IX, X, …)
_ROMAN_NUMERAL_ONLY = re.compile(
    r"^(?:M{0,4})(?:CM|CD|D?C{0,3})(?:XC|XL|L?X{0,3})(?:IX|IV|V?I{0,3})$", re.I
)

# Isolated numbers / credit values / marks (e.g. "10", "05", "100", "3.5")
_ISOLATED_NUMBER = re.compile(r"^[\d.,]+$")

# Grade cell patterns like "S 10", "E 05", "A+", "O+"
_GRADE_CELL = re.compile(r"^[A-Za-z][+-]?\s*\d{0,3}$")

# Parenthesised short values like "(CP)", "(P)", "(F)"
_PAREN_SHORT = re.compile(r"^\(\s*[A-Za-z]{1,4}\s*\)$")

# Credit/mark numeric ranges like "0-100", "0 - 10"
_NUMERIC_RANGE = re.compile(r"^\d+\s*[-–]\s*\d+$")

# Table-row fragment: starts with a pipe, or contains " | " separator (table cell delimiter)
_TABLE_ROW_FRAGMENT = re.compile(r"^\||\s\|\s")

# Keywords that ALWAYS signal a real section heading when present at the start
_SEMANTIC_HEADING_KEYWORDS = re.compile(
    r"^(?:"
    r"introduction|objectives?|attendance|promotion|regulations?|evaluation|"
    r"scholarship|eligibility|fee|rules?|circular|notification|workshop|"
    r"report|internship|hostel|admissions?|results?|seminar|conference|"
    r"placement|examination|grading|credit|marks?\s+distribution|"
    r"library|facilities|infrastructure|anti.?ragging|code\s+of\s+conduct|"
    r"grievance|disciplinary|examination\s+scheme|teaching\s+scheme|"
    r"curriculum|syllabus|miscellaneous|general|academic\s+calendar|"
    r"annexure|appendix|chapter|section|unit|module"
    r")\b",
    re.I,
)


def _is_table_cell_or_noise(text: str) -> bool:
    """
    Return True when the text looks like a table cell, grade value,
    single letter, credit value, page number, or other structural noise
    that should NEVER become a standalone chunk heading.
    """
    n = _normalize_whitespace(text)
    if not n:
        return True
    # Single letter
    if _SINGLE_LETTER.fullmatch(n):
        return True
    # Isolated number
    if _ISOLATED_NUMBER.fullmatch(n):
        return True
    # Roman numeral only
    if _ROMAN_NUMERAL_ONLY.fullmatch(n) and len(n) <= 6:
        return True
    # Grade cell (e.g. "S 10", "E 05", "A+")
    if _GRADE_CELL.fullmatch(n):
        return True
    # Parenthesised short value
    if _PAREN_SHORT.fullmatch(n):
        return True
    # Numeric range
    if _NUMERIC_RANGE.fullmatch(n):
        return True
    # Table-row fragment
    if _TABLE_ROW_FRAGMENT.search(n):
        return True
    # Very short (1-2 chars) with no alphabetic content
...
# ---------------------------------------------------------------------------
# Minimum word count for a genuine heading text body
# ---------------------------------------------------------------------------
_MIN_HEADING_WORDS = 1  # heading itself can be one word; content must be longer


# Patterns for lines that should never become section-boundary headings
# - numbered list items ("1. The Dean...", "2. The Dean...")
# - sub-bullets ("a)", "b)")
# - reference number fragments ("No.SBIN0020262).")
# - signature closing lines ("Addl. CONTROLLER OF EXAMINATIONS CONTROLLER")
_NUMBERED_LIST_ITEM = re.compile(
    r"^(?:"
    r"\d+[.)\s]\s+[a-z]"
    r"|\d+[.)\s]\s+(?:The|All|Copy)\b"
    r"|[a-z][.)\s]\s+"
    r"|No\.\s*\w+[/\w-]*\)?\.?$"
    r"|(?:Addl\.?|Additional|Joint|Deputy|Asst\.?)\s+(?:CONTROLLER|REGISTRAR|DIRECTOR)\b"
    r"|CONTROLLER\s+OF\s+EXAMINATIONS"
    r")",
    re.I,
)
...
def _looks_like_heading(text: str, previous_blank: bool, next_blank: bool) -> bool:
    ...
    Scoring rules:
      - Hard-rejects table cells, grade values, isolated numbers.
      - Hard-rejects numbered list items inside body paragraphs.
      - Requires score >= 2 across positive signals.
    """
    normalized = _normalize_whitespace(text)
    if not normalized or len(normalized) > 80:
        return False
    ...
    # Hard reject: table cells, grade values, isolated numbers, roman numerals …
    if _is_table_cell_or_noise(normalized):
        return False

    # Hard reject: numbered list items / sub-bullets / reference fragments
    if _NUMBERED_LIST_ITEM.match(normalized):
        return False
    ...
    # --- Positive signals (multi-signal scoring) ---
    score = 0

    # Numbered section header — only when followed by a blank line OR contains a keyword
    # e.g. "5. Attendance Rules" but NOT "5. The Principals shall submit:"
    numbered_m = re.fullmatch(r"(\d+(?:\.\d+)*)\s*[.)]\s*(.+)", normalized)
    if numbered_m:
        body = numbered_m.group(2).strip()
        # Only treat as section heading if body matches a keyword
        if _SEMANTIC_HEADING_KEYWORDS.search(body) or _is_all_caps_heading(body):
            score += 3
        # Otherwise skip — it's a numbered list item

    # Ends with colon and surrounded by whitespace
    if normalized.endswith(":") and (previous_blank or next_blank):
        score += 2

    # ALL-CAPS heading (already filters noise inside _is_all_caps_heading)
    if _is_all_caps_heading(normalized):
        score += 2

    # Title-case, short, preceded by blank line, not a numbered list item
    words = normalized.split()
    if (previous_blank and next_blank
            and len(words) <= 8
            and normalized == normalized.title()
            and not re.match(r"^\d", normalized)):
        score += 1

    # Contains a known semantic heading keyword at the start
    if _SEMANTIC_HEADING_KEYWORDS.search(normalized):
        score += 2

    # Must reach a minimum confidence threshold
    return score >= 2
...
# ---------------------------------------------------------------------------
# Table-block detection — groups of lines that form a table
# ---------------------------------------------------------------------------

def _detect_table_blocks(lines: list[LineRecord]) -> list[tuple[int, int]]:
    """
    Detect contiguous line ranges that look like table rows.

    A table block is a run of >= 2 consecutive lines where each line
    either:
      - is a short (<=5 words) all-caps or numeric fragment (typical table cell), OR
      - is a single letter / number / grade value / roman numeral, OR
      - comes immediately after/before such a line with no blank separator.

    Returns a list of (start_index, end_index) ranges (inclusive).
    """
    in_table = False
    block_start = 0
    blocks: list[tuple[int, int]] = []

    def _is_table_row(t: str) -> bool:
        n = _normalize_whitespace(t)
        if not n:
            return False
        if _is_table_cell_or_noise(n):
            return True
        # Short all-caps / numeric lines typical of table cells
        words = n.split()
        if len(words) <= 3 and all(_is_table_cell_or_noise(w) for w in words):
            return True
        # Lines that look like "From I Semester", "From II Semester" — typical table rows
        if re.match(r"^From\s+(I{1,3}|IV|V|VI|VII|VIII|\d)\s+Semester", n, re.I):
            return True
        # Lines containing only grade letters and numbers ("A 10", "B+ 8")
        if re.fullmatch(r"[A-Za-z][+-]?\s+\d+", n):
            return True
        return False

    i = 0
    while i < len(lines):
        row = _is_table_row(lines[i].text)
        if row and not in_table:
            in_table = True
            block_start = i
        elif not row and in_table:
            if i - block_start >= 2:        # at least 2 consecutive table-ish rows
                blocks.append((block_start, i - 1))
            in_table = False
        i += 1
    if in_table and len(lines) - block_start >= 2:
...
def _in_table_block(
    ...
    """Return True when the given line index falls inside a detected table block."""
...
_MIN_CHUNK_WORDS = 15  # chunks with fewer words are merged into predecessor


def _merge_small_chunks(chunks: list[Chunk]) -> list[Chunk]:
    """
    Post-process: merge chunks that are too small (< _MIN_CHUNK_WORDS words)
    into their immediate predecessor.  If the first chunk is tiny and there is
    no predecessor, it is merged into the successor instead.
    """
    if not chunks:
        return chunks
    result: list[Chunk] = []
    for chunk in chunks:
        ...
        if wc < _MIN_CHUNK_WORDS and result:
            ...
            result[-1] = Chunk(
                ...
                metadata={
                    **prev.metadata,
                    "word_count": _word_count(merged_content),
                    "merged_small_chunk": True,
                },
            )
        else:
            result.append(chunk)
    # Edge case: first chunk is tiny — merge forward into second if available
    if result and _word_count(result[0].content) < _MIN_CHUNK_WORDS and len(result) > 1:
        first = result.pop(0)
        merged_content = (first.content + "\n" + result[0].content).strip()
        result[0] = Chunk(
            chunk_id=result[0].chunk_id,
            chunk_type=result[0].chunk_type,
            section_title=result[0].section_title,
            page_start=min(first.page_start, result[0].page_start),
            page_end=result[0].page_end,
            content=merged_content,
            metadata={
                **result[0].metadata,
                "word_count": _word_count(merged_content),
...
def _validate_chunk_meaningful(chunk: Chunk) -> bool:
    """
    Return False when a chunk is not a meaningful semantic unit:
      - fewer than _MIN_CHUNK_WORDS words
      - content is a single table cell / grade / letter / number
      - content consists entirely of noise tokens
    """
    wc = _word_count(chunk.content)
    if wc < _MIN_CHUNK_WORDS:
        return False
    # If every non-empty line is table-cell noise, reject
    content_lines = [l for l in chunk.content.splitlines() if l.strip()]
    if content_lines and all(_is_table_cell_or_noise(l) for l in content_lines):
        return False
    return True
...
def _find_markers(lines: list[LineRecord], mode: str) -> list[HeadingMarker]:
    """
    Detect section-boundary markers in line sequence.

    Key rules:
    - Table cells / noise lines are hard-rejected as headings.
    - Lines inside detected table blocks cannot be headings.
    - Numbered list items (distribution lists, sub-bullets) are rejected.
    - Regulations mode: uses keyword + all-caps validation.
    - Generic mode: requires semantic keyword OR strong formatting signal.
    """
    ...
    # Pre-compute table blocks so we can skip table-row lines as headings
    ...
    for index, line in enumerate(lines):
        ...
        # Hard-reject: table cells are never headings
        if _is_table_cell_or_noise(text):
            continue
        # Hard-reject: lines inside a detected table block
        if _in_table_block(index, table_blocks):
            continue
        # Hard-reject: numbered list items / sub-bullets / reference fragments
        if _NUMBERED_LIST_ITEM.match(text):
            continue
        ...
        if mode == "syllabus":
            ...
            if m:
                ...
                # Only treat as section heading if body has a keyword or is ALL-CAPS
                if (_SEMANTIC_HEADING_KEYWORDS.search(body)
                        or (_is_all_caps_heading(body) and len(body.split()) >= 2)
                ) and not _is_table_cell_or_noise(body):
                    marker = "regulation_section", body.upper()
            elif _is_all_caps_heading(text) and len(text.split()) <= 10:
                real_words = [w for w in text.split() if any(c.isalpha() for c in w)]
                if len(real_words) >= 2 or _SEMANTIC_HEADING_KEYWORDS.search(text):
        ...
        if mode == "generic" and chunk_type == "section":
            # Extra gate for generic mode: must match a keyword OR be clearly all-caps
            has_kw = any(kw in section_title.lower() for kw in _GENERIC_KEYWORDS)
            is_caps = _is_all_caps_heading(text)
            has_semantic = bool(_SEMANTIC_HEADING_KEYWORDS.search(text))
            if not (has_kw or is_caps or has_semantic):
                continue
...
class RegulationChunker(_BaseChunker):
    """
    Improved regulation chunker.

    Key enhancements (hardening pass):
    - Table blocks inside a section are kept intact (not split per row).
    - Small chunks (< _MIN_CHUNK_WORDS words) are merged into predecessor.
    - Chunk validation: rejects table-cell-only chunks before saving.
    - Multi-page sections remain together (no page-break forced splits).
    """
    doc_type = DOC_TYPE_REGULATIONS
    ...
    def chunk(self, pages, source_pdf_name, document_type, base_name, chunk_meta):
        ...
        chunks = self._make_chunks(
            lines, markers, base_name, source_pdf_name,
            document_type, chunk_meta, dynamic_banners,
        )
        if not chunks:
            chunks = _fixed_size_chunks(pages, base_name, source_pdf_name, document_type, chunk_meta)
        # Post-process: merge small chunks and validate
        chunks = _merge_small_chunks(chunks)
        chunks = [c for c in chunks if _validate_chunk_meaningful(c)]
        if not chunks:
            chunks = _fixed_size_chunks(pages, base_name, source_pdf_name, document_type, chunk_meta)
        return chunks

    def _make_chunks(
        self, lines, markers, base_name, source_pdf_name,
        document_type, chunk_meta, dynamic_banners=frozenset(),
    ):
        spans = _section_spans(lines, markers)
        if not spans:
            return []

        # Detect table blocks across all lines so we don't split mid-table
        table_blocks = _detect_table_blocks(lines)
        ...
        for start, end, marker in spans:
            # Include ALL lines in span (including blank lines between table rows)
            # but strip banner/noise lines
            span_lines_raw = lines[start:end]
            # Filter only banner noise, preserve blank lines that separate table rows
            span_lines = []
            for ln in span_lines_raw:
                if _is_banner_or_table_noise(ln.text, dynamic_banners):
                    continue
                span_lines.append(ln)

            # Remove leading/trailing blank lines
            while span_lines and _is_blank(span_lines[0].text):
                span_lines.pop(0)
            while span_lines and _is_blank(span_lines[-1].text):
                span_lines.pop()

            if not span_lines:
                continue

            content = "\n".join(ln.text for ln in span_lines).strip()
            hdr_removed = len(span_lines) < len([ln for ln in span_lines_raw if ln.text.strip()])
            page_start, page_end = _page_range_from_lines(span_lines)
            chunk_id = _make_chunk_id(base_name, marker.section_title, used_ids)
            parsing_meta = {
                "parsing_confidence": _compute_parsing_confidence("section_heading", False, content),
...
def _generic_heading_chunks(
    ...
    """
    Generic heading-based chunker.

    Hardening improvements:
    - Table blocks detected and kept intact (no per-row chunks).
    - Small chunks merged into predecessor.
    - Chunk validation rejects table-cell-only chunks.
    - Semantic boundary detection: only splits on meaningful headings.
    """
    dynamic_banners = _build_dynamic_banners(pages)
    lines = _build_line_records(pages)
    ...
    # Detect table blocks so we can keep them intact across span boundaries
    table_blocks = _detect_table_blocks(lines)
    ...
    for start, end, marker in spans:
        # Preserve blank lines within table blocks; strip banner noise
        span_lines = []
        ...
        # Strip leading/trailing blank lines from each span
        while span_lines and _is_blank(span_lines[0].text):
            span_lines.pop(0)
        while span_lines and _is_blank(span_lines[-1].text):
            span_lines.pop()
        ...
        content = "\n".join(ln.text for ln in span_lines).strip()
        hdr_removed = False   # banners already stripped above
        page_start, page_end = _page_range_from_lines(span_lines)
        chunk_id = _make_chunk_id(base_name, marker.section_title, used_ids)
        parsing_meta = {
    ...
    # Post-process: merge small chunks and validate semantic quality
    chunks = _merge_small_chunks(chunks)
    chunks = [c for c in chunks if _validate_chunk_meaningful(c)]
...
def chunk_document(
    ...
    # Stage 1 — Universal Layout Analyzer
    layout_analyzer = LayoutAnalyzer(debug=False)
    layout = layout_analyzer.analyze(source_name=source_json.name, pages=pages)

    # Stage 2 — Universal Block Detector
    block_detector = BlockDetector(debug=False)
    doc_blocks = block_detector.detect(
        source_name=source_json.name,
        layout_lines=layout.to_dict()["lines"],
    )
    ...
    chunked = ChunkedDocument(
        ...
        document_blocks=doc_blocks.to_dict(),
    )
